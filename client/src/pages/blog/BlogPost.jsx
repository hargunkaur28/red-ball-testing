...
export default function BlogPost() {
  ...
  return (
    <SEOLandingLayout>
      <SEOHead
        title={`${post.title} | Red Ball Sports Arena Blog`}
        description={post.metaDescription}
      ...
      <nav className="bg-white border-b border-black/10 px-4 py-3">
        <div className="max-w-3xl mx-auto flex flex-wrap gap-2 text-xs" style={{ fontFamily: "'DM Sans', sans-serif" }}>
          ...
          <Link to="/blog" className="text-[#0D0D0D]/50 hover:text-[#C8102E] transition-colors">Blog</Link>
          <span className="text-[#0D0D0D]/30">/</span>
      ...
      <article className="max-w-3xl mx-auto px-4 py-12">
        <header className="mb-10">
          <div className="flex items-center gap-3 mb-4">
            <span className="text-xs font-semibold text-[#C8102E] uppercase tracking-widest" style={{ fontFamily: "'DM Sans', sans-serif" }}>{post.category}</span>
          ...
          <p className="text-base md:text-lg text-[#0D0D0D]/70 leading-relaxed border-l-4 border-[#C8102E] pl-4" style={{ fontFamily: "'DM Sans', sans-serif" }}>
            {post.intro}
          </p>
        </header>
        ...
        <div className="mt-10 pt-8 border-t border-black/10">
          <div className="bg-[#F9F6F1] rounded-2xl p-6 flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div className="flex-1">
              <p className="font-bold text-[#0D0D0D] text-sm mb-1" style={{ fontFamily: "'DM Sans', sans-serif" }}>Red Ball Sports Arena</p>
            ...
            <Link to="/book-slots" className="bg-[#C8102E] text-white font-bold px-5 py-2.5 rounded-full text-sm hover:bg-[#a50d26] transition-colors whitespace-nowrap" style={{ fontFamily: "'DM Sans', sans-serif" }}>
              Book a Slot
            </Link>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            {related.map(rp => (
              <Link key={rp.slug} to={`/blog/${rp.slug}`} className="group block bg-[#F9F6F1] rounded-xl p-5 hover:shadow-md transition-shadow">
                <p className="text-xs text-[#C8102E] font-semibold mb-1" style={{ fontFamily: "'DM Sans', sans-serif" }}>{rp.category}</p>
                <h3 className="font-bold text-[#0D0D0D] text-sm leading-snug group-hover:text-[#C8102E] transition-colors" style={{ fontFamily: "'DM Sans', sans-serif" }}>{rp.title}</h3>
              </Link>
            ))}
          </div>
        </section>
      ...
      <section className="max-w-5xl mx-auto px-4 pb-6">
        <div className="flex flex-wrap gap-2">
          <Link to="/blog" className="px-3 py-1.5 border border-black/20 rounded-full text-xs text-[#0D0D0D] hover:border-[#C8102E] hover:text-[#C8102E] transition-colors" style={{ fontFamily: "'DM Sans', sans-serif" }}>
            ← All Articles
          </Link>
          {post.tags.map(tag => (
